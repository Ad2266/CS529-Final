...
from search_central_v4 import find_central_triangulation_edges_deterministic_starts_refined
# 1. 读 instance
inst = read_instance("some.instance.json")
...
center_res = find_central_triangulation_edges_deterministic_starts_refined(
    points_xy,
    triangulations_edges,
    max_iterations=50,
    include_medoid_start=True,
    max_expansions_per_pair=50_000,  
    fail_penalty=1_000_000,
...
for T_edges in triangulations_edges:
    seq_path = find_flip_path_edges(points_xy, T_edges, center_edges)
    print("seq_path")
    
    seq_edges = [step.edge for step in seq_path] 

    rounds = pack_sequential_flips_to_parallel(
        points=[Point(x, y) for x, y in points_xy],
        start_edges=T_edges,
        sequential_flips=seq_edges,
    )
