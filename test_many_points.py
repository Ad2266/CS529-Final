...
from sequential_distance_v2 import find_flip_path_edges, shorten_flip_path, find_flip_path_edges_greedy
...
from greedy_path import find_flip_path_edges_greedy_fast, _triangulation_key_from_edges
# 1. 读 instance
inst = read_instance("long.instance.json")
print(len(inst.points_x))
print(len(inst.triangulations))
...
for T_edges in triangulations_edges:
    ...
    #seq_path = find_flip_path_edges_greedy(
    #points_xy,start_edges=T_edges,goal_edges=center_edges,max_steps=50_000,  
#)
    # seq_path = find_flip_path_edges(points_xy, T_edges, center_edges, weight=1.5, max_expansions=30000)
    start_set = set(_triangulation_key_from_edges(start_edges))
    goal_set  = set(_triangulation_key_from_edges(goal_edges))
    lb = len(start_set ^ goal_set) // 2  

    max_steps = min(50_000, 20 * lb)

    seq_path = find_flip_path_edges_greedy_fast(
        points_xy,
        start_edges,
        goal_edges,
        max_steps=max_steps,
    ...
    seq_edges = [step.edge for step in seq_path] 
